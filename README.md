...
백엔드는 시작 시점과 이후 6시간마다 데이터 보관 작업을 실행합니다. 종료된 지
7일이 지난 `DONE`, `FAIL`, `STOP` 작업과 연결된 비전·에러 데이터는 먼저
`data_archives/YYYY/MM/archive_*` 아래의 CSV 파일로 저장된 다음 DB에서
삭제됩니다. `START`, `RUNNING` 작업과 사용자 활동 기록은 자동 삭제하지
않습니다.
